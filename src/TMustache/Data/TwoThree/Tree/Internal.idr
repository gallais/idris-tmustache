...
data Insert : (ltR : key -> key -> Type) ->
              (val : key -> Type) ->
              Extend key -> Extend key -> Nat -> Type where
  ItFits : Tree ltR val l u n -> Insert ltR val l u n
  TooBig : (k : key) -> val k -> Tree ltR val l (Lift k) n -> Tree ltR val (Lift k) u n -> Insert ltR val l u n

insert : TotalStrictOrder ltR =>
         (k : key) -> val k ->
         Semigroup (val k) =>
         ExtendLT ltR l (Lift k) -> ExtendLT ltR (Lift k) u ->
         Tree ltR val l u n -> Insert ltR val l u n
